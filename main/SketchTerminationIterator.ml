...
module SketchTerminationIterator (B: SketchDomainP) (D: SketchDomainRF) =
struct
  ...
  let rec fwdStm funcs env_vars env_feats vars feats p s =
    ...
    | A_assert (b,_) -> 
		  let featsIn = (AbstractSyntax.bExp_hasNoFeat b feats true) in 
	  	  let p2' = if ((List.length featsIn) > 0) then (B.general_filter p b) else (B.filter p b) in
		  p2'
    | A_if ((b,ba),s1,s2) ->
	  let featsIn = (AbstractSyntax.bExp_hasNoFeat b feats true) in 
	  if ((List.length featsIn) > 0) then (
	  	let p1 = B.general_filter p b in 
      	let p1' = fwdBlk funcs env_vars env_feats vars feats p1 s1 in
      	let p2 = B.general_filter p (fst (negBExp (b,ba))) in 
									...
									let p_out = ref (D.bot env_vars env_feats vars feats) in
									 List.iter (fun (x,y) -> 
									 				...
									 				for i = 0 to (List.length dom)-1 do														
														...
														let b1 = 
														if (i==0) then (A_rbinary (A_LESS_EQUAL,(A_var x,(Lexing.dummy_pos,Lexing.dummy_pos)),(A_const (List.nth dom i), (Lexing.dummy_pos,Lexing.dummy_pos))) 	)												
														else if (i==(List.length dom)-1) then (A_rbinary (A_GREATER_EQUAL,(A_var x,ea),(A_const (List.nth dom i), ea)) )
															 else 
															 (A_bbinary (A_AND, (A_rbinary (A_LESS_EQUAL,(A_var x,ea),(A_const (List.nth dom i), ea)), ea), (A_rbinary (A_GREATER_EQUAL,(A_var x,ea),(A_const (List.nth dom i), ea)), ea)) ) 
																		 in 
														(*let cons = Lincons1.make (Linexpr1.make env_vars) Lincons1.EQ in
														...
														(*Format.fprintf !fmt "### b1: %a \n" bExp_print_aux b1;*)
														(*let p_out = D.config_filter_constraint p b1 r in *)
														pin := D.config_filter !pin b1;
														(*let p1 = D.bwdAssign !pin (l,e1) in 
														Format.fprintf !fmt "### p1 merge: %a ###:\n" D.print p1; *)
														(*pin := D.join COMPUTATIONAL p_out !pin; *)															
...
let rec process_cons feats_list env_feats feats ll = 
  ...
  let analyze (vars,stmts,funcs) main =
	...
	let part_list = P.inner !env_feats !feats constraints_list in 
	let part_listT = T.inner !env_feats !feats constraints_list in
(*	
