...
module TupleGuaranteeIterator (B: TUPLEP) (D: TUPLERF) =
struct
          ...
          if (D.isLeq COMPUTATIONAL i' i) then
            if (D.isLeq APPROXIMATION i' i) then begin
              ...
              if !tracebwd && not !minimal then
                Format.fprintf !fmt "i'': %a\n" D.print i'';
              aux i'' (D.filter (bwdBlk property funcs env env_feat vars configs i'' s) b) (n+1)
            end
          else
            let i'' =
              if n <= !joinbwd then
                ...
                D.widen i (D.join COMPUTATIONAL i i') 
            in
            if !tracebwd && not !minimal then
              Format.fprintf !fmt "i'': %a\n" D.print i'';
            aux i'' (D.filter (bwdBlk property funcs env env_feat vars configs i'' s) b) (n+1)
      in
      let i = D.bot env vars configs in
      let p2 = D.filter (bwdBlk property funcs env env_feat vars configs i s) b in 
      let p = aux i p2 1 in
      addBwdInv l p; p 
