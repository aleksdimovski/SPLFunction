...
module SketchTupleTerminationIterator (B: TUPLEP) (D: TUPLERF) =
struct
...
let print_configs l =
  ...
  let analyze (vars,stmts,funcs) main =
	...
	let list_configs = process !feats_feat in 	
	(*print_configs list_configs; *)
	let vars = List.append vars1 !feats in 
    let env_vars = aux vars (Environment.make [||] [||]) in 	

	(*let p_list = ref [] in 
	List.iter (fun el -> List.iter (fun (key,v) -> let fm = StringMap.find key !ItoA.features in 
												   let cons = Lincons1.make (Linexpr1.make env_vars) Lincons1.EQ in
  											   	   Lincons1.set_array cons [| ((Coeff.s_of_int 1), (Var.of_string fm.featId)) |] (Some (Coeff.s_of_int (-v)));
												   let p = P.inner env_vars vars [cons] in p_list:=!p_list@[p]) el													
													) list_configs; 
	
	let b = B.inner env_vars vars list_configs !p_list in 
	(*Format.fprintf !fmt "\n print b : %a :\n" B.print b; *)


    let _ = fwdBlk funcs env_vars !env_feats vars list_configs (fwdBlk funcs env_vars !env_feats vars list_configs (b) stmts) s in
	...
	List.iter (fun el -> List.iter (fun (key,v) -> let fm = StringMap.find key !ItoA.features in 
												   ...
												   let r = R.zero ~domain:s env_vars vars in 
												   r_list:=!r_list@[r]) el													
													) list_configs; 
	
	let d = D.inner env_vars vars list_configs !r_list in  *)
	(*Format.fprintf !fmt "\n print d : %a :\n" D.print d;*)
	
    let i = bwdRec funcs env_vars !env_feats vars list_configs (bwdRec funcs env_vars !env_feats vars list_configs (D.zero env_vars vars list_configs) s) stmts in
	...
	let r_list = D.elems i in 
	let r2_list = List.map2 (fun el r -> let constraints = ref [] in 
							List.iter (fun (key,v) -> let fm = StringMap.find key !ItoA.features in 
												   let cons = Lincons1.make (Linexpr1.make env_vars) Lincons1.EQ in
  											   	   Lincons1.set_array cons [| ((Coeff.s_of_int 1), (Var.of_string fm.featId)) |] (Some (Coeff.s_of_int (-v)));
												   constraints := !constraints @ [cons]
												   ) el; let s = S.inner env_vars vars !constraints in  R.refine r s												
													) list_configs r_list in 
	let d2 = D.inner env_vars vars list_configs r2_list in
	
	Format.fprintf !fmt "Ranking function is: \n%a\n" D.print d2; (*(InvMap.find 1 !bwdInvMap); *)
	Format.fprintf !fmt "\nTermination Result: \nCorrect = {";
	let b = D.defined d2 in 
	List.iter2 (fun el key ->  if key then (Format.fprintf !fmt "\n"; List.iter (fun (key,v) -> Format.fprintf !fmt "%s{%d} " key v) el;)) list_configs b;
	Format.fprintf !fmt " } \n";   
	true
