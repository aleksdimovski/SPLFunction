...
module MakeSketchDomainP (P: PARTITION) : SketchDomainP =
struct
    ...
    let rec aux t cs = match t with
      | Leaf p ->
        ...
        if (P.isBot p') then Leaf (P.bot env_vars vars)
          ...
          if ((List.length (aExp_hasNoFeat e feats true) == 0)) then 
		  	(
